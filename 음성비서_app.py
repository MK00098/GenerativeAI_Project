...
def main():
    ...
    with st.expander("음성비서 프로그램에 관하여", expanded=True):
        st.write(
            "- 음성 비서 프로그램의 UI는 Streamlit을 활용하여 만들었습니다.\n"
            "- STT(Speech-To-Text)는 OpenAI의 Whisper API를 활용했습니다.\n"
            "- 답변은 OpenAI의 GPT 모델을 활용했습니다.\n"
            "- TTS(Text-To-Speech)는 Google Translate gTTS를 활용했습니다."
        )
    ...
    # 사이드바 입력
    with st.sidebar:
        st.session_state["OPENAI_API"] = st.text_input(label="OPENAI API 키", placeholder="Enter Your API Key", type="password")
        st.markdown("---")
    ...
    with col1:
        ...
        if audio is not None and audio.size > 0 and hasattr(audio, "duration_seconds") and audio.duration_seconds > 0 and not st.session_state["check_reset"]:
